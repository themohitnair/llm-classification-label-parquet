...
class Analysis(BaseModel):
    ...
    emotion: Emotion = Field(description="The primary emotional tone of the text")
    delivery: Delivery = Field(description="The style of delivery of the text")
    domain: Domain = Field(description="The domain or category of the text")


SYSTEM_PROMPT = """
You are an expert social media post analyst. 
Classify the given text along these dimensions. 
Always choose exactly ONE label for each dimension. 
If multiple labels seem possible, pick the one that BEST captures the main intent, tone, or subject. 
Do NOT invent new labels. If unclear, fall back to the closest match.
